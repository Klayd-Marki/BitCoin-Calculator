...
button.addEventListener('click', (event) =>{
    ...
    .then(data => {
        ...
        document.querySelector('.calculation').innerHTML = `BitCoins: ${BitcoinsValue} <br> Currency: ${value} <br> Description: ${data.bpi.EUR.description}  <br> Value in ${value}: ${equation}`;
        }
        if(value == "USD"){
        ...
        document.querySelector('.calculation').innerHTML = `BitCoins: ${BitcoinsValue} <br> Currency: ${value} <br> Description: ${data.bpi.USD.description} <br> Value in ${value}: ${equation}`;
        }
    })
    .catch(error => {
        console.log(error);
